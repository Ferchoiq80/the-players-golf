...
# ==========================================
# CONEXIÓN EN VIVO A GOOGLE DRIVE PARA ESTADÍSTICAS
# ==========================================
...
# ==========================================
# LECTURA LOCAL PARA EL SORTEO
# ==========================================
try:
    df = pd.read_excel('THE PLAYERS.xlsm', sheet_name='Indices', header=5)
    df.columns = df.columns.str.strip()
except Exception as e:
    st.error("❌ No se pudo leer el archivo local. Verifica que 'THE PLAYERS.xlsm' esté en la misma carpeta.")
    st.stop()
...
with st.sidebar:
    ...
    st.markdown("Bienvenido al sistema oficial de The Players.")
    st.markdown("<br>", unsafe_allow_html=True)
    
    iniciar_sorteo = st.button("🎲 Iniciar Sorteo en Vivo", type="primary", use_container_width=True)

# ==========================================
# CABECERA Y CREACIÓN DE PESTAÑAS
# ==========================================
html_cabecera = f"""
<div style="text-align: center; margin-top: 10px; margin-bottom: 10px;">
    <h1 style="color: #002244; font-family: 'Georgia', serif; margin: 0; padding: 0; white-space: nowrap; font-size: 2.2rem;">⛳ Club Tennis Cúcuta — The Players</h1>
</div>
"""
st.markdown(html_cabecera, unsafe_allow_html=True)

# Pestañas principales de navegación
tab_stats, tab_sorteo = st.tabs(["📊 Estadísticas e Informes", "🎲 Sorteo de Foursomes"])

# ==========================================
# PESTAÑA 1: CONSULTA DE ESTADÍSTICAS EN VIVO
# ==========================================
with tab_stats:
    st.header("📊 Tablas de Posiciones y Resultados")
    
    try:
        excel_file = cargar_datos_drive()
        
        opcion = st.radio(
            "Selecciona lo que deseas consultar:",
            ["🏆 Players Championship (Acumulado)", "⛳ Jugadores y Handicaps", "💰 Premios y Resultados"],
            horizontal=True
        )
        st.divider()

        if opcion == "🏆 Players Championship (Acumulado)":
            st.subheader("Leaderboard General")
            df_acumulado = pd.read_excel(excel_file, sheet_name=0)
            st.dataframe(df_acumulado, use_container_width=True)

        elif opcion == "⛳ Jugadores y Handicaps":
            st.subheader("Listado de Jugadores e Índices")
            df_jugadores = pd.read_excel(excel_file, sheet_name="jugadores")
            st.dataframe(df_jugadores, use_container_width=True)

        elif opcion == "💰 Premios y Resultados":
            st.subheader("Histórico de Premios")
            df_premios = pd.read_excel(excel_file, sheet_name=2)
            st.dataframe(df_premios, use_container_width=True)

    except Exception as e:
        st.error(f"No se pudieron cargar los datos de Google Drive. Detalle: {e}")

# ==========================================
# PESTAÑA 2: SORTEO DE FOURSOMES
# ==========================================
with tab_sorteo:
    st.markdown(f"""
    <div style="text-align: center; margin-bottom: 15px;">
        <div style="background-color: #e8f4f8; border-left: 5px solid #002244; border-right: 5px solid #002244; padding: 5px 15px; border-radius: 5px; display: inline-block;">
    ...
    if iniciar_sorteo:
        ...
        try:
            ...
            rango_modificado.api.Orientation = 0
